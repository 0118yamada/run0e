...
###取得したデータから任意のmodle_nameとOIDの情報のみをとってくる################################################
...
def on_off_window(row,sens_name,oid):
    def on_click():
        ...
        #bt_on.configure(style='TButton', background='green')

    def off_click():
        check = var.get()
        result_off_window(check)
        snmp_set_off(check, oid, ch_name)
        #bt_off.configure(style='TButton', background='gray80')
...
def fix_window(row,sens_name,oid):
    ...
    txt.pack()
    #txt.insert(tk.END, "2.5")  # 現在の値を取得する。
...
def create_gui():
    n_sens = 0
    frame_grid = []
    d_sens = {0:107, 1:103, 2:106, 3:104, 4:108, 5:102, 6:109, 7:101, 8:206, 9:207}
